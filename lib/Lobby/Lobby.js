...
class Lobby {
    ...
    createChatrooms(){
        ...
        const totalChatrooms = Math.ceil(totalUsers/this.minChatroomSize);

        // initialize chatrooms with GUIDS
        let roomsGUIDS = []; // store for easy access
        for (let i = 0; i < totalChatrooms; i++) {
            const guid = generateGUID();
            this.chatrooms[guid] = new Chatroom();
            roomsGUIDS.push(guid);
        }

        usernames.forEach((username, index) => {
            const roomIndex = index % totalChatrooms; // Determine the room index in a round-robin fashion
            const currentRoomGuid = roomsGUIDS[roomIndex]; // Use the easy array from the previous block
            const userObject = this.users[username]; // Fetch the user object by username
